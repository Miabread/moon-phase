...
const props = defineProps<{ page_i: number }>();
</script>
...
<template>
    <Card>
        <CardHeader>
            <CardTitle>
                <span class="flex flex-row items-center gap-5 text-nowrap font-normal text-lg">
                    <component :is="pageIcons[props.page_i]" :style="{ color }" />
                    <h2>Page {{ props.page_i + 1 }}</h2>
                    <PercentProgress :percent="trinketUnlockPercents[props.page_i]!" :color="color" />
                </span>
            </CardTitle>
        </CardHeader>
        <CardContent class="flex flex-col gap-4">
            <Table>
                <TableBody>
                    <TableRow v-for="(row, row_i) in TRINKETS_PAGES[props.page_i]" :key="row_i">
